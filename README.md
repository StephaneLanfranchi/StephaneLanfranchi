# Hi, I'm Stéphane 👋

### Senior iOS Engineer · UIKit · Swift · Objective-C · Native App Platforms

I build the native layer behind large-scale app generation platforms: the systems that turn configuration and content into real, performant, maintainable iOS apps.

---

## About me

I'm a senior iOS engineer focused on foundational native work: reusable UI architecture, shared component libraries, server-driven interfaces, and long-lived codebases where Swift and Objective-C need to work together cleanly.

I enjoy working close to the platform, solving problems around maintainability, performance, compatibility, and architecture at scale.

---

## What I work on

| Area | Focus |
|---|---|
| **Native UI architecture** | Reusable UIKit components, design models, layouts, and content configuration systems. |
| **App generation platforms** | Native infrastructure powering configurable apps across many projects. |
| **Platform foundations** | WidgetKit, CloudKit, WKWebView integrations, app lifecycle, internal tooling, and compatibility layers. |
| **Legacy and modern codebases** | Swift and Objective-C interoperability, progressive modernization, and long-term maintainability. |

---

## Tech stack

![Swift](https://img.shields.io/badge/Swift-FA7343?style=for-the-badge&logo=swift&logoColor=white)
![Objective-C](https://img.shields.io/badge/Objective--C-438EFF?style=for-the-badge&logo=apple&logoColor=white)
![UIKit](https://img.shields.io/badge/UIKit-000000?style=for-the-badge&logo=apple&logoColor=white)
![WidgetKit](https://img.shields.io/badge/WidgetKit-000000?style=for-the-badge&logo=apple&logoColor=white)
![CloudKit](https://img.shields.io/badge/CloudKit-007AFF?style=for-the-badge&logo=icloud&logoColor=white)
![WKWebView](https://img.shields.io/badge/WKWebView-000000?style=for-the-badge&logo=safari&logoColor=white)

---

## Focus areas

```swift
let focus = [
    "Reusable UI systems",
    "UIKit component libraries",
    "Server-driven interfaces",
    "Swift and Objective-C interoperability",
    "WidgetKit",
    "CloudKit sync",
    "WKWebView integration",
    "Internal developer tooling",
    "Long-term maintainability"
]
